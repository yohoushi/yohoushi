module GraphsHelper
  ...
  def graph_uri_for(graph, graph_parameter = @list_graph_parameter)
    if Settings.proxy
      if graph.complex
  ...
  def select_tag_for_term(graph_parameter = @list_graph_prameter)
    html = ''
  ...
  def select_tag_for_size(graph_parameter = @list_graph_parameter)
    html = ''
